...
class TrimmedMean(FedAdam):
    ...
    def agg(self):
        ...
        top_beta = torch.topk(client_grad, k=self.args.ATTACKER_PER_ROUND, dim=0).values.sum(dim=0)
        low_beta = -torch.topk(-client_grad, k=self.args.ATTACKER_PER_ROUND,
                               dim=0).values.sum(dim=0)
        agg_client_grad = (client_grad.sum(dim=0) - top_beta - low_beta) / (
            len(self.client_sample_num) - 2 * self.args.ATTACKER_PER_ROUND)
...
class Krum(FedAdam):
    ...
    def agg(self):
        ...
        client_other_grad = torch.stack(self.client_other_grad, dim=0)
        client_grad = torch.cat([client_item_grad, client_other_grad], dim=-1)
        num_clients = len(self.client_sample_num)
        ...
        select_client_idx = torch.argmin(sum_client_scores)
        agg_client_grad = client_grad[select_client_idx]
        agg_client_item_grad = agg_client_grad[:client_item_grad.shape[1]]
        agg_client_other_grad = agg_client_grad[client_item_grad.shape[1]:]
...
class MultiKrum(FedAdam):
    ...
    def agg(self):
        ...
        topk_client_scores = torch.topk(client_scores,
                                        ...
                                        largest=False).values
        sum_client_scores = torch.sum(topk_client_scores, dim=-1)
        selected_clients = torch.topk(sum_client_scores,
                                      k=num_clients - self.args.ATTACKER_PER_ROUND,
                                      largest=False).indices

        selected_grad = torch.index_select(client_grad, dim=0, index=selected_clients)
...
class NormBound(FedAdam):
    ...
    def agg(self):
        ...
        client_other_grad = torch.stack(self.client_other_grad, dim=0)
        client_grad = torch.cat([client_item_grad, client_other_grad], dim=-1)
        client_grad_norm = (client_grad**2).sum(dim=-1, keepdim=True).sqrt()
        clip_coef = torch.clamp(client_grad_norm / self.args.NORM_BOUND, min=1.0)
        client_grad /= clip_coef

        agg_client_grad = torch.matmul(client_weight, client_grad)
        agg_client_item_grad = agg_client_grad[:client_item_grad.shape[1]]
        agg_client_other_grad = agg_client_grad[client_item_grad.shape[1]:]
