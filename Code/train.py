...
def parse_args():
    ...
    parser.add_argument("--AGG_TYPE",
                        type=str,
                        choices=[
                            'FedAdam', 'TrimmedMean', 'Krum', 'MultiKrum', 'NormBound',
                            'MultiKrumUNION', 'NormBoundUNION'
                        ],
                        default='FedAdam')
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        choices=['LabelFlip', 'FedAttack', 'Gaussian', 'LIE', 'Fang', 'ClusterAttack'])
...
# %%
full_attacker_list_path = os.path.join(args.CACHE_DIR, f'{args.DATA}-full-attacker-list.pkl')
...
if os.path.exists(full_attacker_list_path):
    ...
    full_attacker_list = random.sample(range(args.NUM_USERS), k=int(0.05 * args.NUM_USERS))
...
attacker_list_path = os.path.join(args.CACHE_DIR,
                                  f'{args.DATA}-attacker-list-{args.ATTACKER_RATIO}.pkl')
if os.path.exists(attacker_list_path):
    ...
    attacker_id_list = random.sample(full_attacker_list,
                                     k=int(args.ATTACKER_RATIO * args.NUM_USERS))
...
for i in tqdm(range(args.MAX_ROUND)):
    round_loss, round_acc, round_attacker, attacker_grad_norm, \
        total_grad_norm, filter_stat, attacker_loss = orch.update_one_round(i)

    if (i + 1) % args.LOG_ROUND == 0:
        print('Round: {}, train_loss: {:.5f}, acc: {:.5f}'.format(i + 1, round_loss, round_acc))
