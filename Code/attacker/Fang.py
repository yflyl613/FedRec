...
class Fang():
    ...
    def prepare(self, server_model, *args):
        ...
        with torch.no_grad():
            ...
            self.other_grad_mean = torch.mean(total_other_grad, dim=0)
            self.other_grad_std = torch.std(total_other_grad, dim=0)
            self.other_grad_sign = torch.sign(self.other_grad_mean)

            total_item_grad = torch.stack(total_item_grad, dim=0)
            self.item_grad_mean = torch.mean(total_item_grad, dim=0)
            self.item_grad_std = torch.std(total_item_grad, dim=0)
            self.item_grad_sign = torch.sign(self.item_grad_mean)
    ...
    def update(self, server_model, client_id):
        ...
        random_scale = random.uniform(3, 4)
        client_item_grad = self.item_grad_mean + \
            random_scale * self.item_grad_sign * self.item_grad_std
        client_other_grad = self.other_grad_mean + \
            random_scale * self.other_grad_sign * self.other_grad_std
